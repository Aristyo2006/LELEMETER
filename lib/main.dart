...
class LightMeterApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DynamicColorBuilder(
      builder: (lightDynamic, darkDynamic) => _AppTheme(
...
/// Only rebuilds MaterialApp when theme-relevant state changes,
/// NOT on every sensor reading (which fires ~10Hz).
class _AppTheme extends StatelessWidget {
  final ColorScheme? lightDynamic;
  final ColorScheme? darkDynamic;
  const _AppTheme({required this.lightDynamic, required this.darkDynamic});

  @override
  Widget build(BuildContext context) {
    final p = context.select<ExposureState, (ThemeMode, bool, Color)>(
      (s) => (s.themeMode, s.isPureBlack, s.primaryColor),
    );

    final brightness = p.$1 == ThemeMode.system
        ? MediaQuery.platformBrightnessOf(context)
        : (p.$1 == ThemeMode.dark ? Brightness.dark : Brightness.light);
    final dynamicAccent = brightness == Brightness.dark
        ? darkDynamic?.primary
        : lightDynamic?.primary;

    if (dynamicAccent != null) {
      // Safe: setDynamicAccent does notifyListeners only if color changed
      context.read<ExposureState>().setDynamicAccent(dynamicAccent);
    }
